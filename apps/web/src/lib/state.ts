...
export function completeLesson(state: GameState, level: Level, index: number, correct: number, total: number): GameState {
  ...
  return {
    ...
    stats: { ...state.stats, exercises: (state.stats.exercises ?? 0) + total, correct: (state.stats.correct ?? 0) + correct },
  };
}
